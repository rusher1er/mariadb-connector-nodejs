...
class Packet {
  ...
  readBinaryDate() {
    ...
    const year = ((this.buf[this.pos] & 0xff) | (this.buf[this.pos + 1] & 0xff)) << 8;
    const month = this.buf[this.pos + 2];
    const day = this.buf[this.pos + 3];
    let hour = 0;
    let minutes = 0;
    let seconds = 0;
    let microSeconds = 0;
    ...
    if (len > 4) {
      ...
      if (len > 7) {
        microSeconds =
          (this.buf[this.pos + 7] & 0xff) +
          ((this.buf[this.pos + 8] & 0xff) << 8) +
          ((this.buf[this.pos + 9] & 0xff) << 16) +
          ((this.buf[this.pos + 10] & 0xff) << 24);
      }
    }
    const date = new Date(year, month - 1, day, hour, minutes, seconds, microSeconds / 1000);
    this.pos += len;
    return date;
  }
  ...
  readBinaryTime() {
    ...
    const negative = this.buf[this.pos];
    const days =
      (this.buf[this.pos + 1] & 0xff) +
      ((this.buf[this.pos + 2] & 0xff) << 8) +
      ((this.buf[this.pos + 3] & 0xff) << 16) +
      ((this.buf[this.pos + 4] & 0xff) << 24);

    const hours = this.buf[this.pos + 5];
    const minutes = this.buf[this.pos + 6];
    const seconds = this.buf[this.pos + 7];
    let microSeconds = 0;
    if (len > 7) {
      microSeconds =
        (this.buf[this.pos + 8] & 0xff) +
        ((this.buf[this.pos + 9] & 0xff) << 8) +
        ((this.buf[this.pos + 10] & 0xff) << 16) +
        ((this.buf[this.pos + 11] & 0xff) << 24);
    }
    const date = new Date(0, 0, days, hours, minutes, seconds, microSeconds / 1000);
    this.pos += len;
    return date;
  }
